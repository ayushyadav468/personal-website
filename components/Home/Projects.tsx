...
const PROJECT_CARD_DATA: ProjectData[] = [
	{
		...
		description:
			'An e-commerce website developed using ReactJs, NodeJS, Express and MongoDB. Deployed on vercel',
	...
	},
	{
		...
		heading: 'Hulu Clone',
		description:
			'Hulu Clone made using NextJS, TypeScript and TailwindCSS. Deployed on vercel',
...
const ProjectCard = ({ data }: { data: ProjectData }) => {
	return (
		<div className='px-3 py-3 md:px-0 md:py-0 flex flex-col gap-4 border border-gray md:border-none md:flex-row md:gap-0 rounded md:rounded-none relative'>
			<div
				className={`flex-1 ${data.id % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}
			>
				<Image
					src={data.imageUrl}
					alt={data.heading}
			...
			<div
				className={`md:px-8 flex flex-col justify-center items-start gap-2 md:border md:border-gray rounded-md flex-1 ${
					data.id % 2 === 0
						? 'md:order-1 md:pr-[8.5%]'
						: 'md:order-2 md:pl-[8.5%]'
				}`}
			>
				<p className='uppercase text-primary text-sm font-bold tracking-wider'>
					{data.type}
				</p>
				<h3 className='mt-1 mb-2 md:mb-4 text-2xl font-semibold tracking-wide'>
					{data.heading}
				</h3>
				<p className='mb-2 md:mb-4 text-text text-sm font-normal tracking-wide'>
					{data.description}
				</p>
				<div className='flex flex-col md:flex-row md:items-center gap-4 md:gap-6'>
					<Link
						href={data.githubUrl}
						target='_blank'
						className='text-primary text-sm md:text-base font-bold md:tracking-wider hover:text-primary-dark'
					>
						Code on github
					</Link>
					<Link
					...
					>
						<p className='text-primary text-sm md:text-base font-bold md:tracking-wider'>
							See Live Version
						</p>
						<svg
	...
	return (
		<section className='px-6 py-10 md:px-0' id='projects'>
			<div className='max-w-screen-xl mx-auto'>
				<h2 className='text-s-450 font-bold tracking-wide'>Projects</h2>
				<div className='pt-10 pb-0 md:py-10 flex flex-col gap-10 md:gap-40'>
					{PROJECT_CARD_DATA.map((cardData) => {
						return <ProjectCard data={cardData} key={cardData.id} />;
					})}
				</div>
			</div>
		</section>
