	...
	return (
		<header
			className={`w-full fixed top-0 z-50 ${
				navbarScrolled || showMenu
					? 'bg-white shadow-lg'
					: 'bg-ghost-white shadow-none'
		...
		>
			<div className='w-full md:max-w-screen-xl mx-auto py-4 px-2 relative md:px-0 flex justify-between items-center'>
				<Link href={'/'} className='flex items-center gap-4'>
					<LogoSvg />
					<p className='text-xl font-medium'>Ayush Yadav</p>
				</Link>
				<ul className='hidden md:flex md:justify-end md:items-center md:gap-8 md:text-base md:font-medium md:text-text'>
					<li className='border-b-2 border-b-transparent hover:border-b-black transition-all duration-700'>
						<Link
							href={'/#home'}
							scroll={false}
							aria-label={'scroll to home page top'}
						>
							Home
						</Link>
					</li>
					...
					<li>
						<Link
							href={'/#contact'}
							scroll={false}
							className='py-2 px-6 border-2 text-primary font-medium border-primary rounded-3xl hover:text-white hover:bg-primary transition-all duration-300'
				...
				>
					<span
						className={`inline-block w-8 h-1 bg-black transition-all duration-200 ${
							showMenu ? 'rotate-45 translate-y-2' : null
						} `}
					></span>
					<span
						className={`inline-block w-6 h-1 bg-black transition-all duration-200 ${
							showMenu ? 'opacity-0' : null
						} `}
					></span>
					<span
						className={`inline-block w-8 h-1 bg-black transition-all duration-200 ${
